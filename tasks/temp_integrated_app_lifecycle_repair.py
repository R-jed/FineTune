...
path = "FineTune/Models/AudioApp.swift"
s = read(path)
s = replace_once(s, """    func hash(into hasher: inout Hasher) {
        hasher.combine(id)
    }
...
path = "FineTune/Models/VolumeState.swift"
s = read(path)
s = replace_once(s, """    private func ensureState(for pid: pid_t, identifier: String) {
        if states[pid] == nil {
...
path = "FineTune/Views/Rows/AppRowControls.swift"
s = read(path)
s = replace_once(s, """            .frame(width: sliderWidth)
...
""", """    private func invalidateTapsWithChangedProcesses(_ apps: [AudioApp]) {
        let appsByPID = Dictionary(uniqueKeysWithValues: apps.map { ($0.id, $0) })
