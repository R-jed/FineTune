...
@Suite("Device reorder drag state")
struct DeviceReorderDragStateTests {
    @Test("downward midpoint crossing swaps once and resets drag origin")
    func downwardCrossing() {
        var state = DeviceReorderDragState()
        state.update(uid: "A", rawTranslation: 23)
    ...
    func fastDownwardMove() {
        var state = DeviceReorderDragState()
        state.update(uid: "A", rawTranslation: 70)
    ...
    func upwardCrossing() {
        var state = DeviceReorderDragState()
        state.update(uid: "C", rawTranslation: -23)
    ...
    func exactMidpointDoesNotSwap() {
        var state = DeviceReorderDragState()
        state.update(uid: "A", rawTranslation: 22)
    ...
    func resetClearsState() {
        var state = DeviceReorderDragState()
        state.update(uid: "A", rawTranslation: 15)
        state.reset()

        #expect(state.draggedUID == nil)
